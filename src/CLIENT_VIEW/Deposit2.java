...
import static CLIENT_VIEW.Withdraw2.acc;
 ...
 * @author reyes
 */
public class Deposit2 extends javax.swing.JPanel {
    
    private Frame frame;
    public static String acc;
    public static String  mobile;
    public Deposit2(Frame frame) {
        initComponents();
        limits();
    ...
    public void forward(String Username, String Password){
        ...
        String sql = "SELECT Acc_number,balance,MobileNum FROM New_Registered_User WHERE Username = ? AND Password = ?";
        ...
        if (rs.next()) {
            ...
            int depbal = BAL + Frame.bal;
             
            
            if (depbal <= 10000000){
                 System.out.println("TURNING BALANCE : " +  depbal);
                 System.out.println("CURRENT BALANCE : " +  balance);
               ...
               JOptionPane.showMessageDialog(null,"Credit Limit Reached! Please input a smaller amount.");
                System.out.println("CURRENT BALANCE: " + BAL);
                System.out.println("DEPOSIT AMOUNT: " + Frame.bal);
    ...
    try {
        ...
        String insertSql = "INSERT INTO quenum (Username, QueNum, Transaction_Type, Amount, Acc_number, MobileNum, Date, Time) VALUES (?, ?, ?, ?, ?, ?,?,?)";
        java.sql.PreparedStatement insertPstmt = con.prepareStatement(insertSql);
        insertPstmt.setString(1, username);
        insertPstmt.setString(2, newQueueNum);
        insertPstmt.setString(3, transactionType);
        insertPstmt.setString(4, String.valueOf(Frame.bal));
        insertPstmt.setString(5, acc);
        insertPstmt.setString(6, mobile);
...
public CLASS_QNUM_CONSTRUCTOR verifyUser(String username, String password) {
    ...
    try {
        ...
        if (rs.next()) {
            ...
            String queNum = generateQueueNumber(username, CLASS_QNUM_CONSTRUCTOR.data.get(1));
