...
import CLIENT_VIEW.QueueTicket;
...
import CCARD_FRAME.CARD;
import javax.swing.SwingUtilities;
/**
 *
 * @author reyes
 */
public class AccApproval_Pending extends javax.swing.JPanel {
    private CLIENT_VIEW.Frame frame;
    public static String employeeID;
    public static CARD card = new CARD();
    public AccApproval_Pending(CLIENT_VIEW.Frame frame) {
        initComponents();
        ...
        try {
            ...
            try (java.sql.Connection con = java.sql.DriverManager.getConnection("jdbc:mysql://localhost:3306/tbaqs", "root", "")) {
                ...
                try (java.sql.PreparedStatement psInsert = con.prepareStatement(insertCompleted)) {
                    ...
                    card.CreditCard(PENDING.getValueAt(row, 0).toString(), accno);
                    Timer timer2 = new Timer(10000, e -> {
                        card.CreditCard("", "");
    ...
    public void delete_user(int row) {
        ...
        try (java.sql.Connection con = java.sql.DriverManager.getConnection(
                "jdbc:mysql://localhost:3306/tbaqs", "root", ""); java.sql.PreparedStatement psDelete = con.prepareStatement(deleteSql)) {
            psDelete.setString(1, PENDING.getValueAt(row, 0).toString());
            psDelete.executeUpdate();
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private void initComponents() {
        ...
        ER.setIcon(new javax.swing.ImageIcon(getClass().getResource("/BUTTON IMAGES/fill.png"))); // NOI18N
        ER.setBorderPainted(false);
        ER.setContentAreaFilled(false);
        ER.setPressedIcon(new javax.swing.ImageIcon(getClass().getResource("/BUTTON IMAGES/hover_trans.png"))); // NOI18N
        ER.setRolloverIcon(new javax.swing.ImageIcon(getClass().getResource("/BUTTON IMAGES/hover_trans.png"))); // NOI18N
        ...
        background.setIcon(new javax.swing.ImageIcon(getClass().getResource("/PANEL IMAGES/EM_AAP.gif"))); // NOI18N
        add(background);
        background.setBounds(0, 0, 1300, 700);
    }// </editor-fold>//GEN-END:initComponents
    private void ERActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ERActionPerformed
        frame.setContentPane(new AccApproval_Registered(frame));
        frame.revalidate();
    ...
    private void PENDINGMousePressed(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_PENDINGMousePressed
        ...
        if (x != -1) {
            ...
            Object[] options = {"Approve", "Dismiss", "Cancel"};
            int choice = JOptionPane.showOptionDialog(null, "Do you want to approve the application of " + user + "?", "Account Approval", JOptionPane.DEFAULT_OPTION, JOptionPane.QUESTION_MESSAGE, null, options, options[0]);
            if (choice == 0) {
                insert_user(x);
            } else if (choice == 1) {
